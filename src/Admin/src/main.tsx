...
function UserInvestigationView({ investigation, onBack }: { investigation: Investigation; onBack: () => void }) {
  ...
  const useCurrentPosition = () => {
    if (!navigator.geolocation) { setUploadMessage('Enheten stöder inte hämtning av position.'); return; }
    setUploadMessage('Hämtar enhetens position…');
    navigator.geolocation.getCurrentPosition(position => {
      setFindingLatitude(position.coords.latitude.toFixed(7));
      setFindingLongitude(position.coords.longitude.toFixed(7));
      setUploadMessage('Positionen hämtades från enheten.');
    }, error => setUploadMessage(error.code === error.PERMISSION_DENIED ? 'Åtkomst till position nekades.' : 'Enhetens position kunde inte hämtas.'), { enableHighAccuracy: true, timeout: 10000, maximumAge: 30000 });
  };
